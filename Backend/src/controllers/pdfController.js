...
// helper: determine price for a date
...
exports.generateMonthlyBill = async (req, res) => {
  try {
    ...
    if (!month || !year)
      return res
        .status(400)
        .json({ msg: "month & year are required (month 1-12)" });
    ...
    if (customer.area) {
      const areaDoc = await Area.findById(customer.area).lean();
    ...
    // fetch price history
    const settings = await Settings.findOne({ key: "milk_price_history" }).lean();
